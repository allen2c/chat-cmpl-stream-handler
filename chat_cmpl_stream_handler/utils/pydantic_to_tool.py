...
PydanticToolHandlerFn = Callable[[BaseModelT, Any], Awaitable[str]]
...
def pydantic_to_tool(
    ...
    *,
    ...
    tool_description = description if description is not None else inspect.cleandoc(model.__doc__ or "")
...
def build_pydantic_tools_and_invokers(
    configs: Sequence["PydanticToolConfig[Any]"],
...
class PydanticToolConfig(Generic[BaseModelT]):
    """Configuration for exposing one Pydantic-backed tool."""

    model: Type[BaseModelT]
    invoker: PydanticToolHandlerFn[BaseModelT]
    name: Optional[str] = None
    description: Optional[str] = None
...
def _make_pydantic_tool_invoker(
    ...
    async def _invoke(tool_call: ChatCompletionMessageToolCall, context: Any) -> str:
        validated_input = model.model_validate_json(tool_call.function.arguments or "{}")
